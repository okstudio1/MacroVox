...
Client wiring for managed Deepgram grants is complete: the renderer requests one
per streaming or batch call through `deepgram-grant`, via `lib/deepgramCredential.ts`.
BYOK remains the path that needs no sign-in. See the [fix ledger](SECURITY_ARCHITECTURE_FIXES_2026-09-15.md) for current status.

## Runtime contracts

`POST /.netlify/functions/deepgram-grant` requires an allowed Origin, a valid
Supabase bearer token, and a `pro` or `team` subscription. It atomically
reserves one `deepgram_grant` quota unit via `reserve_api_quota`, then calls
Deepgram's grant endpoint with the server key. A successful response is:

```json
{"access_token":"short-lived value","expires_in":60}
```
...
The 60 second grant lifetime controls the connection handshake. Deepgram does
...
Claude (`claude-proxy`), managed Deepgram grants (`deepgram-grant`), and the
legacy audio proxy (`deepgram-proxy`) all call `reserve_api_quota`. The
database takes a transaction-scoped advisory lock, counts the current window,
and inserts the reservation in one transaction. Storage errors return 503 and
do not call the provider. The `api_usage.service` CHECK constraint must list
every value these three functions pass; `api-usage-services.test.ts` checks
this by reading both sides from disk.
...
Deepgram proxy, Deepgram grant, checkout, and billing portal allowlists
...
3. Configure the Supabase URL, service role key, Anthropic key, and Deepgram
   key in Netlify.
4. Apply `202609150001_backend_security_baseline.sql`. This must land before
   step 5 enables `deepgram-grant`, or its quota reservations violate the
   `api_usage.service` CHECK constraint and every grant request fails closed.
5. Deploy checkout, portal, webhook, Claude proxy, and Deepgram grant changes.
