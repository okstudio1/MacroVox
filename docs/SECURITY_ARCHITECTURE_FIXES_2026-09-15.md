...
**Pending approval:** the new client request that sends the Supabase sign-in token to the existing `macrovox.tech` backend for a temporary Deepgram grant is not implemented. Automatic approval review requires explicit authorization for that credential destination. The backend endpoint is prepared, shared-key reads are removed, and managed speech must fail closed until the wiring is approved and completed. BYOK remains the testable path. The exact Windows-origin CORS correction was separately approved and is complete. Do not release this intermediate branch as a completed managed-client upgrade.
...
| --- | --- | --- |
| 1. Shared provider keys readable by subscribers | Remove client key-table reads and webhook key provisioning. Quarantine legacy key storage. The temporary-grant endpoint is implemented; client routing awaits approval. Managed Claude remains server-side. | Token endpoint rejects unauthenticated/unentitled callers, reserves quota, returns only temporary grants with `no-store`. Rotation and deployed permission checks remain required. |
...
Existing managed Claude requests use the Supabase sign-in token to authenticate to the MacroVox backend. The prepared Deepgram grant endpoint also verifies that token, identity, and entitlement; the new client call remains pending the approval described above. Only service-role server operations may mutate quotas, billing reservations, or entitlements. Client-side plan labels are not authorization.
