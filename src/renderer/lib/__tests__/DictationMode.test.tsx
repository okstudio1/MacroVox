...
vi.mock('../auth', () => ({
  ...
  getManagedKeys: harness.getManagedKeys,
...
describe('DictationMode stop lifecycle', () => {
  beforeEach(() => {
    ...
    harness.getManagedKeys.mockReset().mockResolvedValue({ success: false, hasManagedKeys: false })
  ...
  it('revokes managed recording access when the auth session is cleared', async () => {
    ...
    harness.getManagedKeys.mockResolvedValue({ success: true, hasManagedKeys: true })
    render(<DictationMode />)
    const button = await readyRecordButton()

    harness.getUser.mockResolvedValue({ success: false })
    harness.getManagedKeys.mockResolvedValue({ success: false, hasManagedKeys: false })
  ...
  it('never passes the managed entitlement sentinel to a native transcription command', async () => {
    localStorage.removeItem('user_deepgram_key')
    harness.getUser.mockResolvedValue({
      success: true,
      user: {
    ...
    await screen.findByText('Managed transcription is unavailable in this build. Add a personal Deepgram key in Settings.')
    expect(harness.startDeepgram).not.toHaveBeenCalled()
  ...
  it('uses the latest selected transcription mode when the hotkey starts recording', async () => {
    ...
    await screen.findByRole('button', { name: 'Stop recording' })
    expect(harness.startDeepgram).toHaveBeenCalledWith('user-key')
    expect(harness.startRecording).not.toHaveBeenCalled()
  })
